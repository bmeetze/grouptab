...
- The owner's workspace (`~/Documents/Claude/Projects/Group Trip Expense Splitter/`,
  ...
  build ledger. The spec is mirrored there — update both copies on behavior changes.
...
- **Writes to expenses/shares go through RPCs only** (`add_expense`,
  ...
  widen a grant without re-running the adversarial questions in the schema comments.
...
- Non-idempotent writes take a **synchronous ref lock** (`if (ref.current) return;
  ref.current = true`) released in `finally` — and for inserts, only after
  `await refetch()` so stale UI can't double-fire (see `handleMarkPaid` in Settle).
...
- `comments` carries a denormalized `trip_id` (realtime filtering + RLS without
  joins). Tables use `replica identity full` so realtime DELETE events carry
  enough data — keep that when adding tables.
- A twice-weekly GitHub Action (`keepalive.yml`) pings the database so the Supabase
  free tier never pauses. If the project is renamed/moved, update its URL.
